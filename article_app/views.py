...
from .models import Setting, Category, Article, Comment
...
class ArticleList(ListView):
    ...
    def get_queryset(self):
        ...
        return qs.annotate(Count('comment')).filter(is_published=True, is_standalone=False)
...
class RSS(Feed):
    ...
    def link(self):
        return reverse('article_app:rss')
...
class CategoryRSS(RSS):
    ...
    def link(self):
        return reverse('article_app:category_rss', args=[self.category.slug])
...
class ArticleCommentsRSS(Feed):
    ...
    def link(self):
        return reverse('article_app:article_comments_rss', args=[self.article.slug])
...
def e404(request):
    return render(request, 'article_app/404.html', {}, status=404)
